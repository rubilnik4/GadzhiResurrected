...
namespace GadzhiModules.Infrastructure.Implementations.Converters
{
    ...
    public class ConverterClientFilesDataToDTO : IConverterClientFilesDataToDTO
    {
        ...
        {
            var filesRequestExist = await Task.WhenAll(filesData?.FileDatas?.Where(file => _fileSystemOperations.IsFileExist(file.FilePath))?.
            ...
            {
                Id = filesData.Id,
                FileDatas = filesRequestEnsuredWithBytes.ToList(),
            };
        }

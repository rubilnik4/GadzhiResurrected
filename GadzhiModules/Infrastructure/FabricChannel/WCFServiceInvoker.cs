...
namespace GadzhiModules.Infrastructure.FabricChannel
{
    ...
    public class WCFServiceInvoker : IServiceInvoker
    {
        ...
        {
            var configException = new ConfigurationErrorsException(
                string.Format("No client endpoint found for type {0}. Please add the section " +
                "   <client><endpoint name=\"myservice\" address=\"http://address/\" binding=\"basicHttpBinding\" contract=\"{0}\"/></client> in the config file.",
                serviceContractType));
            ...
            {
                if (element.Contract == serviceContractType.FullName)
                {
